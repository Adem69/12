...
{
		$sql="UPDATE coach SET id=:id, nomprenom=:nomprenom,email=:email,specialite=:specialite,age=:age,image=:image WHERE id=:id";
...
try{		
  ...
  print_r($datas);
	...
	function recherchercoach($id){
		$sql="SELECT * from coach where id like '%".$id."%' ";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
